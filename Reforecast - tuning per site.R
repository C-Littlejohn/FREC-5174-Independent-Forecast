...
for (s in 1:length(focal_sites)){
  ...
  # ── Tune ONCE per site on all available training data ──────────────────────
  set.seed(100)
  split     <- initial_split(site_target, prop = 0.80)
  ...
  nee_recipe <- train_nee |> 
    ...
    step_naomit(air_temperature, precipitation_flux,
                surface_downwelling_shortwave_flux_in_air, nee_lag)
  ...
  # Finalize workflow with best hyperparameters — reused for all ref datetimes
  final_workflow <- 
    nee_wflow |> 
    finalize_workflow(best_hyperparameters)
  
  # Evaluate on held-out test set (once per site)
  nee_fit <- final_workflow |> fit(data = train_nee)
  predictions <- predict(nee_fit, new_data = test_nee)
  pred_test <- bind_cols(test_nee, predictions)
  
  multi_metric <- metric_set(rmse, rsq)
  metric_table <- pred_test |> multi_metric(truth = nee, estimate = .pred)
  print(metric_table)
  ...
  for(rd in 1:length(ref_datetime)){
    ...
    # Re-fit using the tuned workflow on the trimmed training window (fast — no tuning)
    nee_full_fit <- final_workflow |> 
      fit(data = training_data_trimmed)
    
    # Process uncertainty: residual SD from training fit
    train_preds <- predict(nee_full_fit, new_data = training_data_trimmed)$.pred
    resid_sd <- sd(training_data_trimmed$nee - train_preds, na.rm = TRUE)
    ...
    for (t in 1:length(datetimes)) {
      ...
      for(ens in 1:n_members){
        print(paste("site:", focal_sites[s], "| rd:", rd, "| t:", t, "| ens:", ens))
        if(met_ens_id <= 30){
